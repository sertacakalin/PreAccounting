...
import { getCustomerSummaryReport } from '@/services/reportService';
...
const PersonalReport = () => {
  ...
  const handleGenerateReport = async (e: React.FormEvent) => {
    ...
    try {
      const data = await getCustomerSummaryReport(fromDate, toDate);
      setReport(data);
    } catch (err) {
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md">
      <h2 className="mb-4 text-2xl font-semibold">My Personal Report</h2>
